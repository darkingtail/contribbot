...
41 个 MCP 工具 + 10 个 Skills — 任务管理、上游追踪、Issue/PR 工作流、多项目总览。
...
安装后自动获得 10 个 Skills + MCP Server（`contribbot-mcp`）。Skills 提供引导式工作流，MCP Server 提供 41 个工具。
...
```
tools/
├── core/      23 个 — contribbot 独有（todo、upstream、knowledge、config）
├── linkage/    4 个 — GitHub 操作 + 本地数据联动（issue_create、pr_create...）
└── compat/    14 个 — GitHub API 封装，保证开箱即用
```
...
```
contribbot/
├── packages/mcp/     # contribbot-mcp（npm 包，41 个工具）
│   └── src/
│       ├── core/     # 工具（core/linkage/compat）、存储、客户端、工具函数
│       └── mcp/      # MCP Server 入口 + 工具注册
├── skills/           # 10 个 Skills（MCP 工具编排层）
