...
```yaml
- text: "研究 Cascader showSearch + loadData 共存方案"
  ...
  pr: 42                             # 关联 PR
  branch: "fix/cascader-search"      # 工作分支
  created: "2026-03-05"
```
...
Phase 2: Skills（工具编排模板）    🔲 把 Prompts 升级为可执行 workflow
Phase 3: Agents（自主决策）        🔲 内置 LLM，自主调用 Skills
```

| Phase | 状态 | 说明 |
|-------|------|------|
| 1. Tools | ✅ | 38 tools + 1 resource + 4 prompts，寄宿于 Claude Code 等宿主 |
| 2. Skills | 🔲 | MCP 之上的工作流编排层，Tools 和 Agent 之间的桥梁 |
